...
def parse_users(
    input_folder: str = "api_example/data/raw/",
    output_folder: str = "api_example/data/processed/",
) -> None:
    """
    ...
    if existing_columns:
        df = df[
            existing_columns
        ]  # Фильтруем только существующие колонки из желаемого списка

    # Чистка данных
    if "id" in df.columns:
        df = df.drop_duplicates(subset=["id"]).reset_index(
            drop=True
        )  # Удаляем дубликаты по идентификатору пользователя
    ...
    df.to_parquet(
        output_folder + "users_clean.parquet", index=False
    )  # Parquet для эффективности


if (
    __name__ == "__main__"
):  # Создает необходимые директории и запускает обработку данных.
    import os

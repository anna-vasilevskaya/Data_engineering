...
def load_osf_users(api_url: str, max_pages: int = 50, page_size: int = 10) -> list:
    ...
    for _ in tqdm(
        range(max_pages), desc="Fetching OSF users (pages)"
    ):  # Цикл по страницам со шкалой прогресса
        ...
        if response.status_code != 200:  # Проверяем успешность запроса
            print(f"WARNING! Status code for URL {next_url} is {response.status_code}.")
            break
        ...
        for item in data_items:
            attributes = item.get(
                "attributes", {}
            )  # Создаем запись с ID и всеми атрибутами пользователя
        ...
        links = payload.get(
            "links", {}
        )  # Получаем ссылку на следующую страницу из метаданных ответа
        next_url = links.get("next")  # URL следующей страницы или None если последняя
        params = (
            None  # Очищаем параметры, так как next_url уже содержит все необходимое
        )

        if (
            not data_items and not next_url
        ):  # Получаем ссылку на следующую страницу из метаданных ответа
            break
...
users = load_osf_users(
    API_URL, max_pages=50, page_size=10
)  # Загружаем данные пользователей из API
...
if users:
    ...
    df.to_parquet(
        "api_example/data/raw/users.parquet", index=False
    )  # Parquet для эффективности

    print(df.info())  # Выводим информацию о структуре данных

    if (
        "full_name" in df.columns
    ):  # Показываем примеры данных - имена пользователей если есть столбец full_name
        print(df["full_name"].head(10))  # Первые 10 имен пользователей
    else:
        print(
            df.head(10)
        )  # Альтернативно показываем первые 10 записей если нет столбца с именами
else:
